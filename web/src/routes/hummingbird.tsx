...
import frames from '#/hummingbird.json'

export const Route = createFileRoute('/hummingbird')({ component: Hummingbird })
...
function Hummingbird() {
  ...
  return (
    <main className="hummingbird-page">
      <pre className="hummingbird" aria-hidden="true">
        {lines.map((line, k) => (
          <div key={k} dangerouslySetInnerHTML={{ __html: line || '&nbsp;' }} />
        ))}
      </pre>
      <p className="tagline">a rotating hummingbird</p>
    </main>
  )
}
