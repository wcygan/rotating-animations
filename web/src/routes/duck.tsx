...
import frames from '#/duck.json'

export const Route = createFileRoute('/duck')({ component: Duck })
...
function Duck() {
  ...
  return (
    <main className="duck-page">
      <pre className="duck" aria-hidden="true">
        {lines.map((line, k) => (
          <div key={k} dangerouslySetInnerHTML={{ __html: line || '&nbsp;' }} />
        ))}
      </pre>
      <p className="tagline">a rotating low-poly duck</p>
    </main>
  )
}
