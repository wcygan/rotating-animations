...
import frames from '#/goose.json'

export const Route = createFileRoute('/goose')({ component: Goose })
...
function Goose() {
  ...
  return (
    <main className="goose-page">
      <pre className="goose" aria-hidden="true">
        {lines.map((line, k) => (
          <div key={k} dangerouslySetInnerHTML={{ __html: line || '&nbsp;' }} />
        ))}
      </pre>
      <p className="tagline">a rotating goose</p>
    </main>
  )
}
