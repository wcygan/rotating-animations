...
import frames from '#/penguin.json'

export const Route = createFileRoute('/penguin')({ component: Penguin })
...
function Penguin() {
  ...
  return (
    <main className="penguin-page">
      <pre className="penguin" aria-hidden="true">
        {lines.map((line, k) => (
          <div key={k} dangerouslySetInnerHTML={{ __html: line || '&nbsp;' }} />
        ))}
      </pre>
      <p className="tagline">a rotating penguin</p>
    </main>
  )
}
