...
import frames from '#/teacup.json'

export const Route = createFileRoute('/teacup')({ component: Teacup })
...
function Teacup() {
  ...
  return (
    <main className="teacup-page">
      <pre className="teacup" aria-hidden="true">
        {lines.map((line, k) => (
          <div key={k} dangerouslySetInnerHTML={{ __html: line || '&nbsp;' }} />
        ))}
      </pre>
      <p className="tagline">a rotating cup of tea</p>
    </main>
  )
}
