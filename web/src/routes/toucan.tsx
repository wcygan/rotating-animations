...
import frames from '#/toucan.json'

export const Route = createFileRoute('/toucan')({ component: Toucan })
...
function Toucan() {
  ...
  return (
    <main className="toucan-page">
      <pre className="toucan" aria-hidden="true">
        {lines.map((line, k) => (
          <div key={k} dangerouslySetInnerHTML={{ __html: line || '&nbsp;' }} />
        ))}
      </pre>
      <p className="tagline">a rotating toco toucan</p>
    </main>
  )
}
